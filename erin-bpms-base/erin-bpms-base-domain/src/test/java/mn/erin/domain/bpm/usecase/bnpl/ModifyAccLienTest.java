...
public class ModifyAccLienTest
{
  private ModifyAccLien useCase;
  ...
  public void setUp()
  {
    ...
    useCase = new ModifyAccLien(directOnlineCoreBankingService);
  }

  @Test(expected = NullPointerException.class)
  public void when_service_null()
  {
    new ModifyAccLien(null);
  }

  @Test
  public void when_response_is_empty() throws BpmServiceException, UseCaseException
  {
    Mockito.when(directOnlineCoreBankingService.modifyAccLien(Mockito.anyMap())).thenReturn(Collections.emptyMap());
  ...
  {
    Mockito.when(directOnlineCoreBankingService.modifyAccLien(Mockito.anyMap())).thenReturn(getDummyResData());
    Map<String, Object> output = useCase.execute(getDummyRequestBody());
    Assert.assertNotNull(output.get("AcctId"));
    Assert.assertEquals(output.get("LienId"), "XB17353");
  }
  ...
  private Map<String, Object> getDummyRequestBody(){
    ...
    requestBody.put("ReasonCode", "999");
