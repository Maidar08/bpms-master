...
public class GetSalaryOrganizationRegNumber extends AbstractUseCase<String, Collection<OrganizationRequest>>
{
  private final OrganizationSalaryRepository organizationSalaryRepository;

  public GetSalaryOrganizationRegNumber(OrganizationSalaryRepository organizationSalaryRepository)
  {
  ...
  public Collection<OrganizationRequest> execute(String regNumber) throws UseCaseException
  {
    ...
    try
    {
      Collection<OrganizationSalary> orgSalary = organizationSalaryRepository.findByRegNumber(regNumber);

      Collection<OrganizationRequest> organizationRequests = new ArrayList<>();

      organizationRequests.addAll(toOrganizationRequests(orgSalary));
