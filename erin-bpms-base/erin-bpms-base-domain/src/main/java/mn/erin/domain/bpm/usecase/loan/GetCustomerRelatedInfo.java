...
public class GetCustomerRelatedInfo extends AbstractUseCase<GetCustomerRelatedInfoInput, Map<String, String>>
{
    private final LoanService loanService;

    public GetCustomerRelatedInfo(LoanService loanService)
    {
    ...
    public Map<String, String> execute(GetCustomerRelatedInfoInput input) throws UseCaseException
    {
        ...
        if (StringUtils.isBlank(input.getRelation()))
        {
            throw new UseCaseException(CUSTOMER_RELATION_TYPE_NULL_CODE, CUSTOMER_RELATION_TYPE_NULL_MESSAGE);
        }

        try
        {
            return loanService.getCustomerRelatedInfo(input.getCustomerCid(), input.getRelation());
        }
