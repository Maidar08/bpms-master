...
public class ErinBpmsLogDbAppender extends DBAppender
{
  ...
  static {
    ...
    try {
      // To get properties from bpms.properties file
      try(InputStreamReader reader = new InputStreamReader(ErinBpmsLogDbAppender.class.getClassLoader().getResourceAsStream("bpms.properties")))
      {
  ...
  public void start() {
    ...
    insertExceptionSQL = log.db.SQLBuilder.buildInsertExceptionSQL(dbNameResolver);
    insertPropertiesSQL = log.db.SQLBuilder.buildInsertPropertiesSQL(dbNameResolver);
    insertSQL = log.db.SQLBuilder.buildInsertSQL(dbNameResolver);
    super.start();
  }
  ...
  {
    stmt.setString(TIMESTMP_INDEX, getSystemDate());
