...
import static mn.erin.domain.bpm.BpmModuleConstants.PROPERTY_ENQUIRE_DOCUMENT_NAME;
 ...
 * @author Zorig
 */
public class DownloadPropertyInfoFromXypTask implements JavaDelegate
{
  private static final Logger LOGGER = LoggerFactory.getLogger(DownloadPropertyInfoFromXypTask.class);
  ...
  public DownloadPropertyInfoFromXypTask(CustomerService customerService,
  ...
  public void execute(DelegateExecution execution) throws Exception
  {
    ...
    String registerNumber = DelegationExecutionUtils.getExecutionParameterStringValue(execution, REGISTER_NUMBER);
    String requestId = (String) execution.getVariable(PROCESS_REQUEST_ID);
    String userId = authenticationService.getCurrentUserId();

    LOGGER.info("*********** Downloading PROPERTY information from xyp system with REG_NUMBER ={}, REQUEST_ID ={}, User ID ={}", registerNumber, requestId,
        userId);
    ...
    if (null == propertyId)
    {
      String message = String.format(PROPERTY_NUMBER_REQUIRED_MESSAGE, registerNumber);
    ...
    citizenInfo.put("regnum", registerNumber);
    ...
    {
      String message = String.format(PROPERTY_INFO_NOT_FOUND_CODE_MESSAGE, registerNumber);
      throw new BpmServiceException(PROPERTY_INFO_NOT_FOUND_CODE, message);
    }

    LOGGER.info("*********** Successful downloaded PROPERTY information from xyp system with REGISTER NUMBER = [{}]", registerNumber);
    ...
    persistPropertyDocument(caseInstanceId, documentId);
  }

  private void persistPropertyDocument(String caseInstanceId, String documentId) throws BpmRepositoryException
  {
    documentRepository.removeBy(caseInstanceId, COLLATERAL_DOCUMENT_CATEGORY, PROPERTY_DOCUMENT_SUB_CATEGORY, PROPERTY_ENQUIRE_DOCUMENT_NAME);
    documentRepository
        .create(documentId, PROPERTY_DOCUMENT_INFO_ID, caseInstanceId, PROPERTY_ENQUIRE_DOCUMENT_NAME, COLLATERAL_DOCUMENT_CATEGORY,
  ...
  private String getDocumentPath(String registerNumber, String requestId)
  {
    ...
    return CUSTOMERS_FOLDER + SLASH + registerNumber + SLASH + requestId + SLASH + PROPERTY_ENQUIRE_DOCUMENT_NAME + "_" + timeStampStr;
  }
}
