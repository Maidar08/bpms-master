...
import mn.erin.domain.bpm.usecase.process.SaveXypSalaries;
import mn.erin.domain.bpm.usecase.process.SaveXypSalariesInput;
 ...
 * @author Tamir
 */
public class DownloadSalaryInfoFromXypTask implements JavaDelegate
{
  private static final Logger LOGGER = LoggerFactory.getLogger(DownloadSalaryInfoFromXypTask.class);
  ...
  public DownloadSalaryInfoFromXypTask(CustomerService customerService,
      AuthenticationService authenticationService, AuthorizationService authorizationService, GroupRepository groupRepository,
  ...
  public void execute(DelegateExecution execution) throws Exception
  {
    ...
    LOGGER.info("*********** Downloading SALARY information from xyp system with REG_NUMBER ={}, REQUEST_ID ={}, User ID ={}", regNum, requestId, userId);

    String month = (String) execution.getVariable(MONTH);
    Integer monthIntValue = Integer.valueOf(month);

    List<SalaryInfo> salaryInfos = getSalaryInfo(regNum, employeeRegNum, monthIntValue);

    Map<String, Object> variables = execution.getVariables();

    if (variables.containsKey(SALARY_INFO))
    {
      execution.removeVariable(SALARY_INFO);
    }

    execution.setVariable(SALARY_INFO, salaryInfos);

    // Save Xyp Salary into table
    String instanceId = (String) execution.getVariable("caseInstanceId");
    SaveXypSalariesInput saveXypSalariesInput = new SaveXypSalariesInput(salaryInfos, instanceId);
    SaveXypSalaries saveXypSalaries = new SaveXypSalaries(processRepository, authenticationService, authorizationService);
    saveXypSalaries.execute(saveXypSalariesInput);

    // Generates salary info document.
    LOGGER.info("############ Generates salary info document with REG_NUMBER ={}, REQUEST_ID ={}, User ID ={}", regNum, requestId, userId);
    ...
    String documentPath = getDocumentPath(regNum, processRequestId);
    ...
    documentRepository.removeBy(caseInstanceId, "04. Лавлагаа", "01. Лавлагаа", SALARY_ENQUIRE_DOCUMENT_NAME);

    documentRepository.create(documentId, "04.01", caseInstanceId, SALARY_ENQUIRE_DOCUMENT_NAME, "04. Лавлагаа", "01. Лавлагаа", documentId, ALFRESCO);

    LOGGER.info("############ Successful generated salary info document.");

    LOGGER.info("*********** Successful downloaded SALARY information from xyp system...");
  }
  ...
  private String getDocumentPath(String registerNumber, String requestId)
  {
    ...
    return CUSTOMERS_FOLDER + SLASH + registerNumber + SLASH + requestId + SLASH + SALARY_ENQUIRE_DOCUMENT_NAME + "_" + timeStampStr;
  }
