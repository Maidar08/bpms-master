...
import {LoanContractWorkspaceServices} from '../services/loan-contract-workspace-services';
...
@Component({
  selector: 'app-loan-contract-case-view',
  template: `
      ...
      <mat-sidenav-content class="content">
        <div class="container">
          <div class="left">
            ...
            <!--                                #mainWorkspace [isCalculation]="isCalculation"></main-workspace-new>-->
  ...
  `,
  styleUrls: ['./loan-contract-case-view.component.scss']
})
export class LoanContractCaseViewComponent implements OnInit {
  @Input() taskId: string;
  @Input() tableData;
  ...
  constructor(private route: ActivatedRoute, private sb: CaseViewSandboxService, private router: Router,
              public workspaceService: LoanContractWorkspaceServices, private auth: AuthenticationSandboxService) {
    ...
    if (null != currentNavigationExtras && null != currentNavigationExtras.data) {
      this.requestType = currentNavigationExtras.data;
      sessionStorage.setItem('requestType', currentNavigationExtras.requestType);
    }
  }
  ...
  onPopState() {
    ...
    if (reqType != null && reqType === 'group-request' ) {
      this.router.navigateByUrl('/loan-page/loan-contract/branch-loan-contract').then(() => {
        sessionStorage.clear();
      });
    } else {
      this.router.navigateByUrl('/loan-page/loan-contract/all-loan-contract').then(() => {
        sessionStorage.clear();
      });
    }
  }
